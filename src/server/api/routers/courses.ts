import { courseLikes } from "@/server/db/schemas/courses/schema";
...
export const courseRouter = createTRPCRouter({
    ...
    addLike: protectedProcedure.input(z.object({courseId: z.number()})).mutation(async ({input, ctx}) => {
        const like = await ctx.db.query.courseLikes.findFirst({
            where: (and(eq(courseLikes.courseId, input.courseId), eq(courseLikes.userId, ctx.user_id)))
        });
