...
async function generateQuiz({
  ...
  try {
    ...
    if (responseBody.questions.length !== 4) {
      await generateQuiz({ requestBody });
      return;
    }
...
await generateQuiz({
  requestBody: { note: "Partial Derivatives Chain Rule" },
});
